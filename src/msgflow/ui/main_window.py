...
QUERY_HELP_TEXT = (
    "About\n"
    "  Filter the run-record list with a small query DSL.\n"
    "  Each clause is `field:value`. Multiple clauses are AND-joined.\n"
    "  A bare token (no `field:`) falls back to the `text` column.\n"
    "\n"
    "Operators (examples on the `text` field)\n"
    "  text:hello  -  contains \"hello\"\n"
    "  text:~^hi.*$  -  regex match\n"
    "  text:=hello  -  exact equals\n"
    "  text:!hello  -  not contains\n"
    "  text:!=hello  -  not equals\n"
    "  text:!~^hi.*$  -  negated regex\n"
    "\n"
    "Available fields (all operators above supported)\n"
    "  " + ", ".join(FIELD_MAP.keys()) + "\n"
    "\n"
    "Quoting\n"
    "  Wrap values with spaces in single quotes: text:'hello world'\n"
    "  Backslash escapes any char. Double quotes are kept as-is,\n"
    "  so JSON regex works directly: trace:~\"name_mark\":\".+axel\"\n"
    "\n"
    "Examples\n"
    "  hello  -  search \"hello\" in text\n"
    "  sender:+86 status:failed  -  sender has \"+86\" AND run failed\n"
    "  trigger:!auto text:~\\d{6}  -  non-auto runs whose text has a 6-digit code\n"
